...
c.add_nondeter_vars(['w1', 'w2', 'w3'],  mean = [0, 0, 0], \
            cov = [[0.02**2, 0, 0], [0, 'sigma2^2', 0], [0, 0, 0.02**2]]) # Set nondeterministic uncontrolled variables
# c.set_assume('x<=200') # Set/define the assumptions
c.set_guaran('P[0.99] (w1 + w2 + w3 <= 0.15)') # Set/define the guarantees
...
start = time.time()
c123_comp.find_opt_refine_param(c, {'p': -1, 'sigma2': -10}, N=200)
end = time.time()

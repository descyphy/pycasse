...
c1.add_nondeter_vars(['w1'],  mean = [0], \
            cov = [[0.1**2]]) # Set nondeterministic uncontrolled variables
...
c2.add_param_vars(['p', 'c', 'sigma2'], bounds = [[0.9, 1], [0, 0.5], [0.01, 0.3]])
...
start = time.time()
c12_comp.find_opt_refine_param(c, [1, 1, 0], N=20)
# c.find_opt_param([1], N=200)
# c2.find_opt_param([-10, 1, -1], N=200)

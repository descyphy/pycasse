...
c.add_nondeter_vars(['w1', 'w2', 'w3', 'w4'],  mean = [0, 0, 0, 0], \
            cov = [[0.02**2, 0, 0, 0], [0, 'sigma2^2', 0, 0], [0, 0, 0.02**2, 0], [0, 0, 0, 0.02**2]]) # Set nondeterministic uncontrolled variables
# c.set_assume('x<=200') # Set/define the assumptions
c.set_guaran('P[0.99] (w1 + w2 + w3 + w4 <= 0.2)') # Set/define the guarantees
...
c1234_comp.checkSat()                  # Saturate c1234_comp
c1234_comp.printInfo()                 # Print c1234_comp

# Find optimal parameters
start = time.time()
c1234_comp.find_opt_refine_param(c, {'p': -1, 'sigma2': -10}, N=200)
end = time.time()

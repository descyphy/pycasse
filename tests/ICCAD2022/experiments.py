...
c.add_param_vars(['p{}'.format(i) for i in range(1, M+1)] + ['sigma{}'.format(i) for i in range(1, M+1)], bounds = [[0.8, 1]]*M + [[0.1, 2]]*M)
...
for j in range(1, M+1):
    ...
    tmp_cov.append(tmp_list)
c.add_nondeter_vars(['n{}'.format(i) for i in range(1, N+1)] + ['param_n{}'.format(i) for i in range(1, M+1)],  
    mean = [0]*(N+M),
...
c.set_guaran('P[0.99] ({} <= {})'.format(expression[3:len(expression)], 1*(N+M))) # Set/define the guarantees
...
print(weights)
composed.find_opt_refine_param(c, weights, N=num_partition)
end = time.time()
